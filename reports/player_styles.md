...
*Generated by `experiments/player_styles/run.py`. Each player is a vector of shot tendencies (serve location, slice/net rates, forehand share, rally length, shotmaking) built from the decoded notation; k-means groups them into archetypes. Style is a continuum, so silhouette scores are modest; treat clusters as soft strata, not hard categories.*
