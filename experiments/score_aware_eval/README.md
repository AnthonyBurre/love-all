...
- **pressure (fine)** — the same tag, but only refining the *full* rally state (the
  strictest test).
- **pressure + lead** — also folds in the games/sets margin (`set_ahead` …
  `set_behind`); richer, so it tests overfitting.

The `pts` column is **server-first**, which the data confirms: server-ahead states
...
to stdout and writes no report or figures; the conclusion below is the result.
...
for the richer `pressure+lead`) yet does **worse** on held-out points: overfitting,
worst for the richest score tag. Keep the pure point-dynamics eval.

## The score effects are real, but they're selection
...
On break points the server wins ~5 points less, and it rises steadily by ~7 points with
match lead. Conditioning on them still doesn't help, because the correlation is
**selection**, not usable signal:
...
2. **The leader is the better player.** Players ahead in the match win more service
   ...
   better. Without knowing *who* is playing, "ahead" is a noisy proxy for quality,
   and folding it into the rally-state backoff fragments cells and overfits (hence
   `pressure+lead` is worst).
...
score. The score matters for **leverage** (which points are worth more), not for
**point-win probability**. That belongs in a layer on top of the eval: a
