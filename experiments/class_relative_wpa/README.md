...
measures each player's decision quality (avg win-prob conceded per stroke); each player
is then compared against **what their own style predicts** instead of against the whole
field, so high-variance stylists aren't penalized for their style, only for being worse
*at* it.
...
fourth decimal**: their benchmark moved under them.
...
The one real choice is how hard that model works. Unregularised, the fingerprint explains
~92% of the variance in shot quality and the residual is mostly noise: the benchmark would
absorb the skill it's meant to measure. So λ is neither a constant nor cross-validated (CV
optimises prediction, which here means absorbing the most). It's solved so the smooth
benchmark accounts for exactly as much variance as the four class means did: the same
style control, without the discontinuity. The two agree at +0.86 (men) / +0.81 (women).
...
Why a benchmark rather than a per-class eval: see `../class_aware_eval`. A class-aware
eval overfits, since the rally state already captures style.
...
- **`reports/class_relative_wpa.csv`** — every player: `avg_wpa_lost`, `archetype`,
  `style_margin` / `style_confident` (from `player_styles`; don't print the archetype
  without checking the flag), `archetype_mean` (retained for reference and
...
`run.py` recomputes these every run, into `reports/class_relative_wpa.md`:
...
over Ostapenko and Parks). That isn't a ranking of shot execution.

**Almost none of its spread is skill.** Split-half reliability ≈0.94 (men) / 0.93
(women), so it measures something stable. But the style fingerprint predicts 0.91 / 0.82
of it *out-of-fold*, so the most that could be reliable non-style signal is the
...
None of this makes the class-relative comparison wrong in principle; it's the only part
