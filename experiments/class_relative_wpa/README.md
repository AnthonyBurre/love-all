...
(from `chess_point_analysis`) measures each player's decision quality (avg win-prob
conceded per stroke); we then compare players **within their style archetype** (from
`player_styles`) instead of against the whole field — so high-variance stylists aren't
penalized for their style, only for being worse *at* it.
