...
def archetype_name(centroid, features) -> str:
    ...
    # The fallback label. Not "All-courter": the women's cluster it lands on has
    # below-average net play (net_pct -0.16) and includes counterpunchers like Halep and
    # Kasatkina.
...
def main() -> None:
    ...
    md.append("*Generated by `experiments/player_styles/run.py`. Each player is a vector of shot "
              "tendencies (serve location, slice/net rates, forehand share, rally length, "
              "shotmaking) built from the decoded notation; k-means groups them into archetypes. "
              "Style is a continuum, so silhouette scores are modest; treat clusters as soft "
              "strata, not hard categories.*")
    md.append("")
    if era_map:
        md.append("*Long, evolving careers are split into early/late **era entities** via the "
                  "`player_eras` layer, so each era clusters on its own (see “Career-era splits” "
                  "below).*")
        md.append("")
    mapping_rows = []
    ...
    for g in ("M", "W"):
        ...
        Z, _, _ = standardize(X)
        # Silhouette is flat (~0.12-0.15) for k = 3..5 but much higher at k = 2 (men 0.362,
        # women 0.506: net-rushers vs the rest for men, slicers vs the rest for women). Finer
        # cuts are a presentation choice over a continuum; 4 matches how the sport talks about
        # styles, and the panel withholds labels with too thin a margin (CONFIDENT_MARGIN).
        k = 4
