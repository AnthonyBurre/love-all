...
predict winners better than the surface-blind model already does? The production model
(`match_charting_project.winprob_match`) drives its score tree from each player's career
serve+return rates with no surface adjustment. Players plainly differ by surface
(Becker's serve rate was 7.8 points higher on hard than clay; Coric leans the other
way), but the *matchup* model may not care: both players' rates shift with the surface,
and only the *relative* shift moves a prediction. This tests whether the relative shift
is big enough, and estimable enough from charted data, to help.

## Design

Both arms use the same analytic score tree and walk-forward evaluation; only the
serve/return rates differ:

- **Baseline** — career rates to date, shrunk toward the tour mean with pseudo-count
  `K=100` (the production `walk_forward_strength` recipe).
...
`k_s` is tuned on matches **before 2020** and judged once on **2020–present**, with
...
(clay and grass are where the signal should be; hard is most of the data and closest
to career-average conditions).
...
- **Charted matches only.** Rates come from the charted sample, which skews to
  ...
  clay sample. Surface counts are thin for most players; the backoff handles that,
  but it means the variant mostly *is* the baseline.
