...
def write_report(res: dict, path: Path) -> None:
    ...
    md.append("*Generated by `experiments/blind_reid/run.py`. The unit is one **performance** (a "
              "single player in a single match), vectorized from the decoded notation with the "
              "name stripped. Scores are **verification AUC** (the chance a same-player pair is "
              "closer than a different-player pair; 0.5 = chance) and **rank-1 accuracy** (is a "
              "performance's nearest neighbour the same player), each computed only on players "
              "held out of the distance metric's fit.*")
    md.append("")
    ...
    md.append("")
    md.append("The **response** block (returns plus rally strokes, nothing about the delivery) "
              "beats the **serve** block outright "
              ...
              "So you wouldn't name your opponent from their serve: it's the *weakest* of the "
              "three views. What gives a player away is what comes back once the point is live.")
    md.append("")
    ...
    md.append("")
    md.append("Each row is the same pair list, filtered. The question is whether the signal is "
              "the player or something correlated with the player.")
    md.append("")
    ...
    md.append("- **Serve resolution.** The serve block is limited to direction, in-rate and "
              "outcome. Real serve identification would use speed, spin and toss, none of which "
              "the notation records, so \"the serve is the weakest block\" is a statement about "
              "*charted* serve data, not about serves.")
    path.write_text("\n".join(md) + "\n")
