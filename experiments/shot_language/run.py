...
def main():
    ...
    md.append("*Generated by `experiments/shot_language/run.py`. Each point is a sentence in a "
              "small shot alphabet; an order-2 Markov model is the 'opening book'. Surprise = "
              "−log₂ P(next shot | last two), in bits; a player's mean surprise is how far their "
              "shot choices stray from tour norms.*")
    md.append("")
    ...
    md.append("So there is **no payoff to surprise**: the relationship is non-monotone and ~0 "
              "overall. Sound, moderately-aggressive shots gain the most; the *most* unexpected "
              "shots slightly lose ground, since they are typically defensive scrambles, not "
              "creative winners (game-state drives the tail, not creativity). Unpredictability "
              "differentiates *who a player is*, not *how well they're playing*.")
    md.append("")
