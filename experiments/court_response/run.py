...
def main():
    ...
    md.append("*Generated by `experiments/court_response/run.py`. The state is the incoming ball "
              "only (its character and the zone it lands in, named relative to the receiver's own "
              "hands), and the response is the player's decision: wing, shot type, and line. Lift "
              "compares the player's response rate in that state to the rest of the field in the "
              ...
              f"in the selection. Support floors: n≥{MIN_STATE} in the state and count≥{MIN_CELL} "
              f"pooled, n≥{FOLD_STATE} and count≥{FOLD_CELL} within each fold, shrunk "
              f"lift≥{LIFT_MIN} where it was found and ≥{VAL_LIFT_MIN} where "
              "it was measured. Patterns are ranked by evidence (count x log2 lift), so a "
              "bread-and-butter tendency outranks a rare quirk with a flashier lift. Uncharted "
              "directions and unknown wings are excluded outright. Each pattern carries its "
              "payoff: the player's point-win rate after playing "
              f"that response (shrunk toward the field's by {K_CONV} pseudo-counts) "
              "next to the field's own rate playing the same response to the same ball. Lift is "
              "the choice; payoff is what it earns.*")
    md.append("")
    ...
    if len(one):
        md.append("**What the winner's curse was worth here.** For the "
                  ...
                  "displayed lift comes from a fold with no vote in the selection), the mean"
                  f" discovery lift is {one.disc_lift.mean():.2f}x and the mean displayed lift is "
                  f"{one.lift.mean():.2f}x, so "
                  f"**{(one.lift.mean() - 1) / (one.disc_lift.mean() - 1):.0%} of the "
                  "discovered edge survives out of sample**. That is close to what "
                  "`rally_patterns` measured on a different screen over different features (50%), "
                  "which is some evidence it is a property of this kind of search rather than of "
                  "either experiment.")
        md.append("")
        md.append(f"The other {len(both):,} patterns cleared from both directions, each "
                  "fold validating the other, and show the two halves pooled, so their displayed "
                  "lift is not a clean out-of-sample read and is not quoted as one. Being "
                  "findable twice independently is itself the stronger claim.")
        md.append("")
    md.append("The correction is cheap here: it takes this table from 2,804 patterns "
              f"over 805 players to {len(df):,} over {df.player.nunique():,}. The split-half r "
              "below is why: these cells were already stable, so correcting them mostly removes "
              "the thin tail rather than the findings.")
    md.append("")
    ...
    for g in ("M", "W"):
        ...
        md.append(f"- {len(prof):,} players clear the gates with at least one "
                  "stable pattern.")
        md.append(f"- **Split-half stability**: r = {stab_r:+.2f} across {len(xs):,} "
                  "player-state-response cells: the tendencies repeat"
                  " in the other half of the same player's matches.")
        if share:
        ...
        md.append("**Off the return** (the server's shot 3, by charted return depth; strongest "
                  "evidence first, one per player):")
        md.append("")
        seen = set()
