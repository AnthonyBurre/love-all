...
attempt counts by context for K in 1..3, and (context -> response) counts for
...
def collect(con, gender: str):
    ...
    while True:
        ...
        for mid, p1, p2, svr, fs, ss, win in batch:
            ...
            for i in range(1, len(pt.shots)):
                pl = names[pt.shots[i].hitter]
                att = 1 if pt.shots[i].terminal in ("*", "@") else 0
                b = base[pl]
                b[2 * h] += 1
                b[2 * h + 1] += att
                for k in KS:
...
def main():
    ...
    a1.set_ylabel("held-out log-loss (attempt prediction)")
    ...
    md.append("*Generated by `experiments/context_length/run.py`. Match-hash split "
              "halves; trigger models predict held-out attempts through a shrinkage "
