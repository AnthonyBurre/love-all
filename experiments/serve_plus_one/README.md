...
service courts, which has a cost.

A wide serve opens the forehand in the deuce court and the backhand in the ad
court. So "mid-depth drive return into the middle" is two situations, arriving
off different serves, at different angles, with the server recovering from
opposite corners. Nadal's pooled reading is a crosscourt forehand
at 1.7x. Split by court, the same description is a **crosscourt forehand on the
deuce side** (n=3,202) and an **inside-out forehand on the ad side**
(n=2,937). The pooled number averages two different shots.
...
The test is about coverage only, and it runs **before any lift is computed**. Picking
the tier that surfaced the most patterns would pick the resolution that flattered the
player, and the replication gate doesn't fully undo that. The cost is visible in the
numbers: 116 men clear the gates on at least one full-tier pattern, but only 90 are
assigned the full tier, because the other 26 got there on a single state.
...
pooled. That's 85% of `court_response`'s `ret` pattern count, two thirds of it
side-specific.
...
answer differs between them. A pooled state can't report these: it names whichever
response won the average. Sampras answers
the same mid-depth return with a backhand volley to the backhand corner on the deuce
side and to the forehand corner on the ad side. Kerber's ad-court wide serve into a
mid-depth return in the middle draws a crosscourt backhand at 3.7x.

**It holds up.** Full-tier split-half r = 0.85 (men) / 0.69 (women) over 9,120
cells, at the finest state, where counts are thinnest.
...
pooled "mid-depth drive return into the middle". The court drawing includes the
serve, which shows the side at a glance.
