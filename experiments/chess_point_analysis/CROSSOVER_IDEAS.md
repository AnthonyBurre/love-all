...
win-probability eval and shot-quality work in this folder. Each treats a point string as
an alternating-turn move list (decoder:
`match_charting_project.shots.notation`).

## Built

In `match_charting_project.shots`:
...
- **Tactical motif mining** *(chess: forks, pins, recurring motifs)* → `../shot_patterns`
  ...
  (serve-and-volley, wrong-foot) is open.
- **Score-aware eval** *(tennis-specific extension)* → `../score_aware_eval`, settled
  ...
  independence). That result is why `../match_winprob` uses an analytic score tree for
  leverage.
...
probabilities ("at net, opponent passing from the deuce corner" → put-away rate). The
notation's placement resolution (zones 1–3, no court position) is too coarse for it as
stated, so it needs reformulating at the resolution the data has.
