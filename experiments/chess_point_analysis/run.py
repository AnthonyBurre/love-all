...
def main() -> None:
    ...
    md.append("*Generated by `experiments/chess_point_analysis/run.py`. Treats each point's shot "
              "notation as a move list, builds an empirical P(server wins) eval, and scores every "
              "stroke by win-probability added (WPA): the centipawn-loss / blunder idea ported to "
              "tennis.*")
    md.append("")
    ...
    md.append("Besides shot direction, the rally state reads **return depth** (7/8/9), **slice vs "
              "drive**, and **net/approach** (+/- modifiers). Raw men's server-win% by each "
              "detail (marginal, so confounded, but the signal is clear):\n")
    dep, rt, ap = contrasts
    ...
    md.append("Lower `avg_wpa_lost` = gives away less per stroke (the centipawn-loss analogue); "
              "`accuracy` rescales it 0–100, monotonically, so it reorders nobody. "
              "`unforced_lost_share` is how much of the loss came from charted *unforced* errors, "
              "the most self-inflicted.\n")
    md.append("**Read this leaderboard as style, not skill.** There is no oracle for the best "
              "stroke, so a loss blends selection, execution, and opponent pressure. Worse, WPA "
              "telescopes within a point, so the per-stroke average is identically *(concession "
              "per point) / (strokes per point)*. That second factor dominates: the metric "
              "correlates ≈ −0.85 with rally length and is ≈ 0.9 predictable from style "
              "out-of-fold, which leaves almost none of its spread as skill. "
              "`experiments/class_relative_wpa` measures this every run and tries to correct for "
              "it.\n")
    for g in ("M", "W"):
