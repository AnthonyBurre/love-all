...
The point eval (``chess_point_analysis``) answers "how is *this rally* going?".
This layer answers "how is the *match* going?" by propagating a single number —
each player's probability of winning a point on their own serve — up the scoring
tree: point -> game -> set -> match. That propagation is exact under the assumption
that points are independent given the server, which the ``score_aware_eval``
experiment validated empirically (conditioning the point eval on the score does not
help). So this model *consumes* the point eval rather than replacing it: the eval's
pre-serve ``base()`` is the population anchor for each player's serve strength, and
the eval's per-shot WPA is later scaled by the *leverage* this layer computes.
...
Closed forms / memoized recursions, standard tennis scoring (game to 4 by 2 with
deuce; set to 6 by 2; 7-point tiebreak at 6-6; best-of-3 or -5). Two documented
approximations, each worth <0.1% WP: (a) the first server of each *new* set is taken
as the alternation of the previous set's first server, and (b) non-standard
historical final-set rules (advantage sets, 10-point match tiebreaks) use the 6-6
tiebreak by default — the deciding-set trigger is overridable (``final_tb_games``)
so specific matches like the 2019 Wimbledon final (12-12) render exactly.
"""
...
def game_wp(g: float, a: int, b: int) -> float:
    ...
    ``g`` is the server's point-win probability. Deuce (both >=3) is the closed
    form g^2 / (g^2 + (1-g)^2) with the two advantage states folded in.
    """
    if a >= 4 and a - b >= 2:
        return 1.0
    if b >= 4 and b - a >= 2:
...
class MatchWP:
    ...
    def __init__(self, p1: float, p2: float, best_of: int = 3,
        ...
        self.final_tb_games = final_tb_games   # games at which the deciding-set TB fires
        self.final_tb_target = final_tb_target
        self.hold1 = game_wp(p1, 0, 0)         # P(player1 holds)
        self.hold2 = game_wp(p2, 0, 0)         # P(player2 holds)
        self._set: dict = {}
        self._tb: dict = {}
        self._match: dict = {}
    ...
    def _tb_server_is_starter(points_played: int) -> bool:
        """In a tiebreak the starter serves point 1, then serve swaps every 2."""
        return ((points_played + 1) // 2) % 2 == 0   # True -> starter serves next point
    ...
    def tb_win(self, a: int, b: int, starter1: bool, target: int = 7) -> float:
        ...
        if a == b and a >= target - 1:
            # Tiebreak "deuce": over the next two points the serve splits one each
            # (player1 serves one, player2 the other), so by the same recursion as a
            # game deuce, D = ab / (ab + (1-a)(1-b)) with a=p1 (serve), b=1-p2 (return).
        ...
        pa = self.p1 if server1 else (1 - self.p2)   # P(player1 wins this point)
    ...
    def set_win(self, ga: int, gb: int, p1_serves: bool, final: bool = False) -> float:
        ...
        if ga >= tb_games and gb >= tb_games:   # >= (not ==) guards against runaway
            target = self.final_tb_target if final else 7
            return self.tb_win(0, 0, p1_serves, target)
        key = (ga, gb, p1_serves, final)
        if key in self._set:
            return self._set[key]
        gw = self.hold1 if p1_serves else (1 - self.hold2)   # P(player1 wins this game)
    ...
    def _next_set_server(self, s: Score) -> bool:
        """Approx first server of the *next* set = alternation of this set's first.

        The first server of the set is invariant within the set: in a normal game
        it is ``p1_serves`` xor the game-parity (both flip each game); in a tiebreak
        the games stay 6-6 while the server flips per point, so the invariant is the
        tiebreak *starter* (who also served the set's first game)."""
        if s.in_tiebreak:
    ...
    def _tb_starter1(self, s: Score) -> bool:
        """Did player1 serve the *first* point of the current tiebreak?"""
    ...
    def _game_wp(self, s: Score, pa: int, pb: int, starter1: bool, target: int) -> float:
        """P(player1 wins the current game/tiebreak) from points (pa, pb)."""
        if s.in_tiebreak:
    ...
    def _compose(self, pgw: float, s: Score, final: bool, nss: bool) -> float:
        """Match WP given P(player1 wins the current game/tiebreak) = pgw."""
        if s.in_tiebreak:
            pset = pgw                      # winning the tiebreak wins the set
        else:
