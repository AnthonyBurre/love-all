...
def main():
    ...
    md.append("*Generated by `experiments/match_winprob/run.py`. An analytic point→game→set→match "
              "win-probability model, driven by each player's serve+return strength (estimated "
              "**walk-forward** from only matches played earlier, so nothing leaks), sitting on "
              "top of the point eval. Validated three ways below.*")
    md.append("")
    ...
    md.append("")
    md.append("Predicted and actual win rates track the diagonal across all deciles. Because each "
              "match is scored only from *earlier* matches, this is an out-of-sample test: no "
              "information from the match or the future leaks into its own prediction.")
    ...
    md.append(f"Federer's win probability peaked at **{peak['wp']:.1%}** serving at "
              "8-7, 40-15 in the fifth (two championship points), then fell away as Djokovic "
              "saved them and won the deciding tiebreak. The lower panel is **leverage**: how "
              "much each point could swing the match.")
    md.append("")
    ...
    md.append("")
    md.append("This prices a mistake in match units rather than point units, the chess-analogue "
              "step: the same error costs far more on a championship point than at 40-0. "
              "**Caveat:** like the point eval, this conflates shot selection, execution, and "
              "opponent pressure; strength is a serve+return rate with no surface or form "
              "adjustment, and players with thin prior charting fall back toward an even matchup.")
    md.append("")
