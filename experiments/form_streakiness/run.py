...
def main():
    ...
    md.append("*Generated by `experiments/form_streakiness/run.py`. Form = shrunk mean of "
              ...
              "days; the form arm shifts matchup point-win probs by `w·(form₁−form₂)` "
              f"with `w` tuned pre-{str(TEST_FROM)[:4]} and judged on {str(TEST_FROM)[:4]}+. The "
              "streaky arm scales each player's form by their "
              "training-era gap-limited residual autocorrelation (test era only; see the README "
              "for why). Same score tree and walk-forward discipline everywhere.*")
    md.append("")
    ...
    md.append("")
    md.append("- **Form signal:** regressing a match's realized residual on the form "
              f"difference entering it gives slope **{slope:.2f} ± {se:.2f}** (test era, "
              f"n={int(test.sum()):,}), so recent residuals do carry some "
              "information about the next charted match.")
    md.append("- **Streakiness signal:** per-player residual autocorrelation over "
              f"≤{GAP_DAYS}-day gaps: observed mean **{obs_rho.mean():+.3f}** across "
              f"{len(obs_rho)} players vs permutation-null mean **{null_rho.mean():+.3f}**. The "
              "distributions nearly coincide (right panel)"
              ", so there is little *stable individual* streakiness to modulate by.")
    md.append("")
