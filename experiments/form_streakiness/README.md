...
enough in charted data to help, in two steps that separate two claims:

1. **Form** — does a player's recent performance *relative to expectation* predict
   their next match at all (uniform weight for everyone)?
...
**The form signal.** After every charted match, each player gets a residual: their
observed share of total points won minus the share the baseline model expected
pre-match. That makes it opponent-adjusted: doing better than expected against a
stronger opponent counts as form even in a loss. A player's *form* entering a match is
the shrunk mean of their last 10 residuals within the previous 18 months (pseudo-count
5, so one hot match moves it little).

**The comparison.** Both arms use the same score tree and walk-forward setup. The form
arm shifts the matchup point-win probabilities by `w · (form₁ − form₂)`, symmetrically.
`w` is tuned on pre-2020 matches, judged once on 2020+.
...
would leak). It inherits `w` from the uniform tuning, with no extra test-era knob.
...
- **Charted matches are a sparse, biased sample of a season.** Consecutive
  ...
  later rounds, so a player's charted "recent matches" over-sample their wins at
  big events. Real week-to-week form may exist and be invisible here; this answers
  what *charted* data supports, which is what the site can use.
