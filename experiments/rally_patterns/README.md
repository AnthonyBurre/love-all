...
The opening is blinded because that's where this kind of mining breaks. A deep context
anywhere in the point drifts into the serve, where it has less support than the dedicated
sections and doesn't know the service court, even though a wide serve opens opposite wings
on the two sides. `shot_triggers`' openings section and `serve_plus_one` measure those
shots; this measures what's left.
...
screened for that player (all three depths in one family, since they're one search). The
rate, lift, conversion and green/trap tag are then read off the other fold,
...
at four: seven times as many patterns from a candidate pool only three times larger. They
don't hold up. Those three-shot survivors keep 33% of their discovered edge
...
- **Everything is conditional on the point surviving the opening**, and how often that
  happens varies a lot by player: 17% of Krajicek's points reach the fifth shot against
  53% of Coria's. Blinding doesn't remove the serve from these profiles; it conditions
  on it. Karlovic's rally book is built from the points his serve didn't end. This is
  selection, not noise, and blinding deeper makes it worse. Every profile ships its
  exposure next to it, and comparisons across players inherit the problem in a way
  comparisons within a player do not.
...
three-shot candidates survive, and both belong to retired players who appear in no draw.
The experiment still runs weekly and writes `reports/rally_patterns.csv`, so the tier can
come back if the charting ever supports it.
